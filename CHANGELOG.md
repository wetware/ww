...
- **Glia capability authority and isolation primitives expanded (pre-alpha breakage accepted).** Capability authority now matches by per-instance `cap_id` (not `schema_cid`), `defcap` creates Glia-native capability servers with introspectable descriptors, `attenuate` enforces method allowlists with nested-intersection semantics, and `isolate` accepts explicit typed `:env` imports only (`(cap ...)` and `(data ...)`) in a fresh restricted context. `(data ...)` now admits authority-free closures/macros and rejects authority-bearing ones with captured-offender diagnostics.
