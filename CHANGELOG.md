...
### Removed
- **Obsolete kernel ABI reporting and unused runtime scaffolding.** Removed the
  kernel ABI fingerprint/version fields from builds and `/version`, together
  with disconnected architectural machinery that had no production consumers.
  Current WAGI, RPC, capability, and wire behavior is unchanged.
- **Speculative capability discovery and grant review tooling.** Removed the
  ...
  Constructive explicit child grants, structural validation, and runtime
...
- **Explicit pid0 kernel source resolution.** `ww run --kernel` and
  ...
  `/version`, and fails closed when the selected component is absent or
  malformed.
...
- **Composable call-validity guards.** The membrane now exposes `CallGuard`
  ...
  shared contract. `RevocationGuard` is the targeted-revocation composition
  model; targeted revocation no longer needs a global Atom epoch advance.
...
- **Namespace bootstrap configuration now fails closed.** Namespace bootstrap
  ...
  Kubo-backed boot resolution can mask the configuration error.
...
- **Capability-model docs (`doc/capabilities.md`, `doc/architecture.md`).** Docs describe CID-as-capability, the three attenuation points (membrane graft / root Atom binding / Glia env bindings), epoch-based revocation, structured Glia errors, and "MCP = Glia eval" framing. The "VFS & capability model" section uses an `Atom → CidTree root → guest paths → content` diagram and makes explicit that WASI preopens are a protocol detail rather than a security boundary.
