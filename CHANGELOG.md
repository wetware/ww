...
### Fixed
- **`ww shell` local-daemon connect path now survives bootstrap/export mismatch and stream-start timing races.** Kernel startup now exports a real bootstrap membrane via `system::serve` (instead of run-only wiring), with `KernelBootstrap` forwarding `graft()` once the host membrane is ready; this restores the expected bootstrap capability shape for shell login/graft flows. On the client side, shell stream open now retries within a bounded readiness window and uses a longer RPC timeout to avoid premature failures while daemon-side listeners finish coming up. Also fixed an executor edge case where a dropped kill channel could busy-loop until child exit; kill-watch now disables cleanly and awaits natural process completion.
