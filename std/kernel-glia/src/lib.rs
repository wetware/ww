...
// (see std/kernel-glia/build.rs). `(doc cap)` returns a human-readable summary
        ...
        async move {
            ...
            loop {
                ...
                // never an ambient `{cap}-handler` guest binding.
                {
                    ...
                    // registry baked into the kernel (see std/kernel-glia/build.rs).
...
mod tests {
    ...
    #[test]
    fn help_lists_possession_introspection_without_static_catalog() {
        assert!(HELP_TEXT.contains("(schema cap)"));
        assert!(HELP_TEXT.contains("(doc cap)"));
