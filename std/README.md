...
| `kernel-next/` | Transitional opt-in Rust pid0 -- directly installs the shipped `/status` composition without Glia. |
...
make kernel-next # builds std/kernel-next/bin/main.wasm
make shell     # builds std/shell/bin/shell.wasm
make status    # builds std/status/bin/status.wasm
make std       # builds kernel + kernel-next + shell + status
```
