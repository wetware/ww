//! Discovery guest: two-agent Greeter demo via service-name vat RPC.
//!
//! Demonstrates the full Wetware discovery flow:
//!   build → schema-inject → provide service locator on DHT →
...
//!
//! **`serve`**: provides the greeter service locator on the DHT, discovers
//! peers via `routing.find_providers()`, dials them via `VatClient`, binds the
//! returned `VatConnection`, and calls `greet()`.
...
async fn greet_peer(
    ...
    let greeter: greeter_capnp::greeter::Client = bind_resp.get()?.get_cap().get_as_capability()?;
...
async fn run_service(membrane: Membrane) -> Result<(), capnp::Error> {
    ...
    let dht_key = service_key(&routing, GREETER_SERVICE).await?;
    log::info!("service: peer {}", short_id(&self_id));
    log::info!("service: locator {GREETER_SERVICE}, schema CID {GREETER_CID}");
