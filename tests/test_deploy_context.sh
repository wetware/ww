...
grep -Fq 'mkdir -p wetware/kernel/bin' "$WORKFLOW" \
  || fail "deploy context must create the kernel bin directory"
...
if grep -Fq 'etc/init.d' "$WORKFLOW"; then
  fail "deploy context must not package legacy init.d scripts"
fi
