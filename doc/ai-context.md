...
|----------------|--------------|---------------|
| `vat` | Cap'n Proto RPC | `/ww/0.1.0/vat/{service-name}` listener |
...
Key abstractions:
- **Cell type system**: vat schema metadata comes from the embedded
  `ww.schema.v1` SchemaBundle; `WW_CELL_MODE` envvar indicates
  transport (vat, raw, http).
